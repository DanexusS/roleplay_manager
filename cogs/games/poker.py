...
#
#         user_balance = DB_SESSION.query(User).filter(User.id == f"{ctx.author.id}-{guild.id}").first().balance
#         try:
#             if int(bet) < 10:
...
#
#         DB_SESSION.query(User).filter(User.id == f"{members_ids[small_blind_id]}-{guild.id}").first().balance -= bet // 2
#         DB_SESSION.query(User).filter(User.id == f"{members_ids[blind_id]}-{guild.id}").first().balance -= bet
#
...
#
#         if last_player_id:
#             user = DB_SESSION.query(User).filter(User.id == f"{last_player_id}-{guild.id}").first()
#             user.balance += pot
#             DB_SESSION.commit()
...
#             user = DB_SESSION.query(User).filter(User.id == f"{winner}-{guild.id}").first()
...
#         column = 0
#         for member in members:
#             balance = DB_SESSION.query(User).filter(User.id == f"{member.id}-{guild_id}").first().balance
